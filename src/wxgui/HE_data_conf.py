...
class HEMyDialogRMC(MyDialogRunningMeancentered):
    ...
    def on_add(self, event):
        current_samplerate = self.spin_ctrl_current_samplerate.GetValue()
        entered_window_size = self.spin_ctrl_windowsize.GetValue()
        ostatok = entered_window_size % current_samplerate
        if ostatok:
...
class HEMyFrame(MyFrame, BusyFrame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ad = self.ad_dydt, \
                  self.ad_shift, \
                  self.ad_normalise, \
                  self.ad_running_mean, \
                  self.ad_std_deviation, \
                  self.ad_running_mean_weight, \
                  self.ad_running_mean_centered, \
                  self.ad_abs
    ...
    def update_table(self):
        ...
        for idx, tag in enumerate(self.DATA.tags_list):
            ...
            G.SetCellEditor(idx, NaN_column, gridlib.GridCellTextEditor())
            G.SetCellValue(idx, NaN_column, 'NaN' if self.DATA.data[tag].isnull().any() else '-')
            G.SetCellAlignment(idx, NaN_column, wx.ALIGN_RIGHT, wx.ALIGN_CENTER)
            G.AutoSizeColumn(NaN_column)
        ...
        if self.DATA.tags_list:  # не пустой список тэгов
            ...
            if self.DATA.data.size < 300000 or True:  # TODO here debug 'or True'
    ...
    def on_select_cell(self, event):
        ...
        tags_selected = True if len(self.get_selected_tags()) else False  # there are some rows selected

        if self.choice_derivative.GetSelection() != wx.NOT_FOUND:  # and one of derivative selected
            self.bitmap_button_add_derivative.Enable(tags_selected)
        else:
            self.bitmap_button_add_derivative.Enable(False)

        # if self.DATA.config.get('freq in Secs', 0):  # если задана частота samplerate  - не имеет значения
    ...
    def on_fake_btn(self, event):
        tags = self.get_selected_tags()
        with HEMyDialogFakeData(parent=self) as FakeDataDialog:
            assert isinstance(FakeDataDialog, HEMyDialogFakeData)
            if FakeDataDialog.ShowModal() == wx.ID_CANCEL:
                return  # the user changed their mind
            print("Here we should fake user!")

            for tag in tags:
                try:
                    self.DATA.generate_fitting_data(tag, debug=FakeDataDialog.checkbox_create_faked_tag.GetValue())
    ...
    def on_export_2excel_btn(self, event):
        ...
        with wx.FileDialog(self, "Export to excel", wildcard=XLSX_WILDCARD,
            ...
            try:
                ...
                if not tags:
                    tags = None
                self.busy = "Writing to Excel..."
                self.DATA.export_2excel(pathname, tags_list=tags, debug=True)
                self.busy = False
    ...
    def ad_abs(self):
        """Absolute value"""
        tags = self.get_selected_tags()
        self.busy = "Calculating Absolute value..."
        for tag in tags:
            self.DATA.ad_abs(tag)
    ...
    def ad_running_mean_centered(self):
        ...
        if not self.DATA.config.get('freq in Secs', 0):
            # unknown samplerate
            wx.MessageBox(
                "I do not know the Samplerate!\nFirst set Samplerate with the drum button,\nthen fill the gaps of data.",
                "Unknown parameter",
        ...
        dlg.spin_ctrl_windowsize.SetIncrement(samplerate)
        if self.ss_prev_rm_window_size is not None:
            dlg.spin_ctrl_windowsize.SetValue(self.ss_prev_rm_window_size)
        if dlg.ShowModal() == wx.ID_CANCEL:
            return
        window_size = int(dlg.spin_ctrl_windowsize.GetValue())  # in seconds
        window_size_in_samplesteps = window_size // samplerate
        self.busy = "Calculating Running mean..."
        for tag in tags:
            self.DATA.ad_running_mean_adv(tag, window_size=window_size_in_samplesteps, in_sec=window_size)
        self.ss_prev_rm_window_size = window_size
