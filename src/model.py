...
class Model(object):
    ...
    def window_size(self):
        return self.data_conf.get(WINDOW_SIZE, 0)
    ...
    def loadmodel(self, filename):
        ...
        try:
            ...
            print("KERAS Model loaded from <%s>" % filename)
        except AttributeError:
            print("Error while loading KERAS model from <%s>" % filename)
            return False
        with h5py.File(filename, 'r') as f:
            try:
                self.conf = pickle.loads(a2b(f.attrs['zbv_meta']))
                print("ZBV Model configuration <%s> loaded" % f.name)
            except AttributeError:
                print('Error while loading ZBV model from <%s>' % f.name)
                return False
        self.ss_loaded = True
        self.ss_saved = False
        return True
